...
def get_images_to_explain(y_true,y_pred,n_classes=3,n_images=10):
    ...
    for i in range(n_classes):
        ...
        indices_correct = np.nonzero(y_true[correct]==i)[0][:n_images]
        indices_incorrect = np.nonzero(y_true[incorrect]==i)[0][:n_images]
...
#     print(y_pred.shape,"shappppeee")
    image_ids = get_images_to_explain(test_labels,y_pred)
    for class_id in image_ids:
        print("class",class_id)
        correct,incorrect = image_ids[class_id][0],image_ids[class_id][1]
        for c in correct:
            path = test_list_IDs[c]
            true_id = test_labels[c]
            print(class_id,true_id,type(class_id),type(true_id))
            assert (class_id==true_id)
            img = load_image(path)
            class_idx,heatmap,heatmap_jet,superimposed_image,image_bbox = gradCAM(model,layer_name,img)
            # plt.imsave("./best_models/{0}/correct_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcam_heatmap_jet.jpg".format(name,true_id,class_idx,layer_name,c),heatmap_jet)
            plt.imsave("./best_models/{0}/correct_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcam_superimposed.jpg".format(name,true_id,class_idx,layer_name,c),superimposed_image)

            class_idx,heatmap,heatmap_jet,superimposed_image,image_bbox = gradCAMpp(model,layer_name,img)
            # plt.imsave("./best_models/{0}/correct_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcampp_heatmap_jet.jpg".format(name,true_id,class_idx,layer_name,c),heatmap_jet)
            plt.imsave("./best_models/{0}/correct_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcampp_superimposed.jpg".format(name,true_id,class_idx,layer_name,c),superimposed_image)
        ...
        for c in incorrect:
            ...
            img = load_image(path)
            class_idx,heatmap,heatmap_jet,superimposed_image,image_bbox = gradCAM(model,layer_name,img)
            # plt.imsave("./best_models/{0}/incorrect_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcam_heatmap_jet.jpg".format(name,true_id,class_idx,layer_name,c),heatmap_jet)
            plt.imsave("./best_models/{0}/incorrect_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcam_superimposed.jpg".format(name,true_id,class_idx,layer_name,c),superimposed_image)

            class_idx,heatmap,heatmap_jet,superimposed_image,image_bbox = gradCAMpp(model,layer_name,img)
            # plt.imsave("./best_models/{0}/incorrect_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcampp_heatmap_jet.jpg".format(name,true_id,class_idx,layer_name,c),heatmap_jet)
            plt.imsave("./best_models/{0}/incorrect_true_{1}_pred_{2}_layer_{3}_id_{4}_gradcampp_superimposed.jpg".format(name,true_id,class_idx,layer_name,c),superimposed_image)
